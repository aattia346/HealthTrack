    ...
    throws Exception {
		...
		if(Validation.validateNumber(bookingId) & Validation.checkIfSomethingExists("booking_id", "booking", Integer.toString(bookingId))
			...
			if(B.getUserId()==userId) {		
				...
				WhatsappSender.sendMessage("2" + B.getPhone() , t.write("Your booking has been cancelled"));
				mav.setViewName("/user/profiles/user");
			}
		}else {
			mav.setViewName("/user/login");
    ...
    throws Exception {
		...
		if(user.getId() == userId) {
			...
			if(bookingOwner == userId) {		
				...
				BookingDao.deleteBooking(bookingId);
				WhatsappSender.sendMessage("2" + B.getBookingPhone() , t.write("Your booking has been cancelled"));
				mav.setViewName("/user/profiles/user");
			}
		}else {
			mav.setViewName("/user/login");
