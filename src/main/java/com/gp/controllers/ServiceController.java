...
public class ServiceController {
	...
	public ModelAndView submitBookingClinic(@CookieValue(value = "lang", defaultValue = "en") String cookie,
		...
		if (userId != 0) {
			...
			if (!Validation.validateName(firstName) || !Validation.validateName(lastName)) {
				model.addAttribute("invalidName",
						"<p class=\"wrong-input wrong-input-register-page-1\">" + t.write("Invalid Name") + "</p>");
				errors = true;
			}

			if (!Validation.validateEmail(email)) {
				model.addAttribute("invalidEmail",
						"<p class=\"wrong-input wrong-input-register-page-1\">" + t.write("Invalid Email") + "</p>");
				errors = true;
			}
			if (!Validation.validatePhone(phone)) {
				model.addAttribute("invalidPhone", "<p class=\"wrong-input wrong-input-register-page-1\">"
						+ t.write("Invalid Phone Number") + "</p>");
				errors = true;
			}

			if (!Validation.validateText(msg)) {
				model.addAttribute("invalidmsg", "<p class=\"wrong-input wrong-input-register-page-1\">"
						+ t.write("Your message has invalid characters") + "</p>");
				errors = true;
			}
			if (PersonDao.canPersonBook(userId)) {
				request.setAttribute("limitExceed", " ");
			} else {
				request.setAttribute("limitExceed", "<p class=\"wrong-input wrong-input-register-page-1\">"
						+ t.write("Sorry you have reached the maximum number of bookings per day(3)") + "</p>");
				errors = true;
			}
			...
			if (errors) {
				...
				model.addAttribute("checkYourBooking",
						"<div class=\"alert alert-danger text-center\">"
								+ t.write("Your information have some errors, Please check the booking and resend it")
								+ "</div>");
			...
			model.addAttribute("loginFirst",
					"<div class=\"alert alert-danger text-center\">" + t.write("to book please")
							+ "<a target=\"_blank\" href=\"/HealthTrack/login\">" + t.write("login") + "</a>"
							+ t.write("first or") + "<a target=\"_blank\" href=\"/HealthTrack/signup\">"
							+ t.write("register") + "</a></div>");
			mav.setViewName("user/profiles/clinic");
		}
	...
	public ModelAndView submitBookingDay(@CookieValue(value = "lang", defaultValue = "en") String cookie,
		...
		if (userId != 0) {
			...
			if (!Validation.validateBookDate(serviceId, bookFromAsDate)
					|| !Validation.validateBookDate(serviceId, bookToAsDate)) {
				model.addAttribute("invalidDate", "<p class=\"wrong-input wrong-input-register-page-1\">"
						+ t.write("Sorry this date has been already booked") + "</p>");
				errors = true;
			}
			if (bookFromAsDate.after(bookToAsDate)) {
				model.addAttribute("invalidDate",
						"<p class=\"wrong-input wrong-input-register-page-1\">" + t.write("Invalid Date") + "</p>");
				errors = true;
			}
			if (!Validation.validateText(msg)) {
				model.addAttribute("invalidmsg", "<p class=\"wrong-input wrong-input-register-page-1\">"
	...
	public ModelAndView submitBookingTime(@CookieValue(value = "lang", defaultValue = "en") String cookie,
		...
		if (userId != 0) {
			...
			try {
				...
				model.addAttribute("invalidTime", "<p class=\"wrong-input wrong-input-register-page-1\">"
						+ t.write("Please choose time") + "</p>");
				errors = true;
			}
	...
	public ModelAndView reviewComment(@CookieValue(value = "lang", defaultValue = "en") String cookie,
		...
		if (userId == 0) {
			request.setAttribute("commentLoginFirst",
					"<p class=\"wrong-input wrong-input-register-page-1\">" + t.write("Please login first") + "</p>");
			...
			if (!Validation.validateText(comment)) {
				request.setAttribute("invalidComment", "<p class=\"wrong-input wrong-input-register-page-1\">"
						+ t.write("Invalid characters in your comment") + "</p>");
			} else {
			...
			request.setAttribute("commentsLimitExceeded", "<p class=\"wrong-input wrong-input-register-page-1\">"
					+ t.write("sorry you reached the maximum number of comments for this service") + "</p>");
		}
